    ...
    return (
        <div className={`${wrapperClassName} form-floating mb-3 `}>
            <select
                ...
                disabled={disabled}
            >
                {placeholderOption && <option value="placeholder" disabled selected={selected === ''}>Select A {labelTitle}</option>}
                {
                    arrayOfOptions && arrayOfOptions.map((option, i) => {
                        return <option selected={selected === option.value} value={option.value} key={i}>{option.label}</option>;
                    })
                }
            </select>
